...
def apply_merges(initial_routes, savings, demands, capacity, dist,
    ...
    for s in savings:
        ...
        if not is_endpoint(i, ri) or not is_endpoint(j, rj):
            bad = []
            if not is_endpoint(i, ri):
                bad.append(f"node {i} not an endpoint")
            if not is_endpoint(j, rj):
                bad.append(f"node {j} not an endpoint")
            log.append({"pair": pair_label, "saving": sval, "allowed": False,
...
def kpi_strip(R, active_final, merges_allowed, merges_rejected):
    ...
    avg_util = (
        sum(ra["utilisation"] for ra in R["route_analytics"]) / len(R["route_analytics"])
        if R["route_analytics"] else 0
    )
    kpis = [
    ...
    for col, (color, val, label) in zip(cols, kpis):
        col.markdown(
            ...
            f"padding:1rem 0.8rem;text-align:center;'>"
            f"<div style='font-family:\"DM Mono\",monospace;font-size:1.6rem;"
            f"font-weight:500;color:{color};line-height:1;'>{val}</div>"
            f"<div style='font-size:0.58rem;font-weight:600;letter-spacing:0.16em;"
            f"color:#2e2e3a;text-transform:uppercase;margin-top:0.4rem;'>{label}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
...
def step_header(num, icon, title):
    st.markdown(
        ...
        f"<span style='font-size:0.95rem;font-weight:600;color:#e8e6e0;"
        f"letter-spacing:0.02em;'>{icon} {title}</span>"
        f"</div>",
        unsafe_allow_html=True,
    )
...
def route_card(ra, vehicle_num, demands, time_windows, show_tw):
    ...
    # ── Build tw_block without backslashes inside f-strings ──
    tw_block = ""
    if show_tw and time_windows and ra["arrivals"]:
        ...
        joined = "  ".join(arr_parts[:8])
        ellipsis = "…" if len(arr_parts) > 8 else ""
        tw_block = (
            "<div style='margin-top:0.6rem;font-family:\"DM Mono\",monospace;"
            "font-size:0.65rem;color:#454550;line-height:1.8;'>"
            + joined + ellipsis +
            "</div>"
        )

    # ── Build optional TW status span without backslashes inside f-strings ──
    if show_tw:
        tw_status_val = ra["tw_status"]
        tw_span = (
            "<span style='font-size:0.68rem;color:#454550;'>TW "
            "<span style='color:#39e5b6;font-family:\"DM Mono\",monospace;'>"
            + tw_status_val +
            "</span></span>"
        )
    else:
    ...
    st.markdown(
        ...
        f"<div style='font-family:\"DM Mono\",monospace;font-size:0.9rem;"
        f"color:#e8e6e0;word-break:break-all;'>{path}</div>"
        f"<div style='margin-top:0.7rem;background:#0e0e16;border-radius:2px;"
        ...
        f"<span style='font-size:0.68rem;color:#454550;'>Cost "
        f"<span style='color:{color};font-family:\"DM Mono\",monospace;'>{ra['cost']:.1f}</span></span>"
        f"<span style='font-size:0.68rem;color:#454550;'>Load "
        f"<span style='color:{color};font-family:\"DM Mono\",monospace;'>{ra['demand']:.0f}</span></span>"
        f"<span style='font-size:0.68rem;color:#454550;'>Util "
        f"<span style='color:{color};font-family:\"DM Mono\",monospace;'>{ra['utilisation']:.0f}%</span></span>"
        f"<span style='font-size:0.68rem;color:#454550;'>Time "
        f"<span style='color:{color};font-family:\"DM Mono\",monospace;'>{ra['total_time']:.1f}</span></span>"
        f"{tw_span}"
        f"</div>"
        f"{tw_block}"
        f"</div>",
...
def merge_row(entry, idx):
    ...
    # Build border/background values outside f-strings to avoid backslash issue
    border_color = "#1a2800" if entry["allowed"] else "#2a0000"
    label_bg = "#0a1400" if entry["allowed"] else "#1a0000"
    ...
    st.markdown(
        ...
        f"color:{color};font-weight:600;min-width:14px;margin-top:1px;'>{icon}</span>"
        f"<div style='flex:1;'>"
        f"<div style='display:flex;align-items:center;gap:0.6rem;'>"
        f"<span style='font-family:\"DM Mono\",monospace;font-size:0.8rem;"
        f"color:#e8e6e0;font-weight:500;'>#{idx} {entry['pair']}</span>"
        f"<span style='font-size:0.6rem;letter-spacing:0.12em;text-transform:uppercase;"
        f"color:{color};background:{label_bg};border:1px solid {color};"
        f"border-radius:3px;padding:0.1rem 0.4rem;'>{label}</span>"
        f"</div>"
...
with st.sidebar:
    ...
    st.markdown(
        "<div style='font-size:0.65rem;color:#2e2e3a;letter-spacing:0.16em;"
        "text-transform:uppercase;margin-bottom:0.4rem;'>Core Inputs</div>",
        unsafe_allow_html=True,
    )

    matrix_raw = st.text_area(
        "Distance Matrix", value=EXAMPLE_MATRIX, height=140,
    ...
    st.markdown(
        "<div style='font-size:0.65rem;color:#2e2e3a;letter-spacing:0.16em;"
        "text-transform:uppercase;margin-bottom:0.4rem;'>Advanced Constraints</div>",
        unsafe_allow_html=True,
    )

    enable_tw = st.checkbox("Enable Time Windows (VRPTW)", value=True)
    tw_raw = st.text_area(
        "Time Windows (early,late per node)", value=EXAMPLE_TW, height=120,
        help="One 'early,late' pair per line (or semicolon-separated). First line = depot.",
        disabled=not enable_tw,
    )

    enable_service = st.checkbox("Enable Service Times", value=True)
    service_raw = st.text_area(
        "Service Times (per node)", value=EXAMPLE_SERVICE, height=55,
        help="Time spent servicing each node. First = depot (usually 0).",
        disabled=not enable_service,
    )

    max_route_time = st.number_input(
        "Max Route Duration (0 = unlimited)", min_value=0.0, max_value=9999.0, value=0.0, step=1.0
    )

    enable_priority = st.checkbox("Enable Priority Routing", value=True)
    priority_raw = st.text_area(
        "Customer Priorities (1=low, 2=med, 3=high)", value=EXAMPLE_PRIORITY, height=55,
        help="One value per customer (not depot). 1=low, 2=medium, 3=high.",
        disabled=not enable_priority,
    )
    priority_alpha = (
        st.slider(
            "Priority Weight α", min_value=0.0, max_value=2.0, value=0.5, step=0.1,
            help="How strongly priorities bias the savings ranking.",
        )
        if enable_priority else 0.5
    )
    ...
    st.markdown("""
    <div style='margin-top:1.5rem;padding:1rem;background:#06060a;border:1px solid #1a1a28;
                border-radius:6px;font-size:0.65rem;color:#2e2e3a;line-height:2;'>
...
if st.session_state.result is None and not st.session_state.error:
    ...
    <div style='text-align:center;padding:5rem 1rem;'>
        ...
        <div style='font-size:0.82rem;color:#2e2e3a;margin-top:1rem;max-width:400px;
...
if st.session_state.result:
    ...
    with tabs[0]:
        ...
        st.markdown(
            ...
            "Each customer starts on its own route "
            "<span style='font-family:\"DM Mono\",monospace;color:#c8f135;'>0 → i → 0</span>. "
            "These are the baseline before any merges.</div>",
            unsafe_allow_html=True,
        )
        ...
        for k, route in enumerate(R["initial_routes"]):
            ...
            # Build priority snippet outside f-string to avoid backslash issue
            if pri:
                pri_val = str(pri[route[1] - 1])
                pri_snippet = (
                    "&nbsp;·&nbsp; Priority: "
                    "<span style=\"color:" + color + ";\">" + pri_val + "</span>"
                )
            else:
                pri_snippet = ""
        ...
        sav_df = pd.DataFrame(R["savings"]).rename(columns={
            ...
            "S(i,j)_base": "Base Savings",
            "Priority Factor": "Priority ×",
            "S(i,j)_weighted": "Weighted Savings",
        })
        ...
        sorted_df = sav_df[
            ["Node i", "Node j", "Base Savings", "Priority ×", "Weighted Savings"]
        ...
        for col, active, label, detail in constraints:
    ...
    # ── TAB 2: Final Routes ───────────────────────────────────────────────────
    with tabs[1]:
        num_vehicles = len(R["route_analytics"])
        step_header(5, "🚚", f"Optimised Solution — {num_vehicles} Vehicles")
        st.markdown(
            "<div style='font-size:0.78rem;color:#454550;margin-bottom:1rem;'>"
        ...
        st.markdown(
            ...
            f"padding:1.2rem 1.5rem;margin-top:1rem;display:flex;align-items:center;"
            f"gap:2rem;flex-wrap:wrap;'>"
            f"<div>"
            f"<div style='font-size:0.6rem;color:#454550;letter-spacing:0.16em;"
            f"text-transform:uppercase;'>Initial</div>"
            f"<div style='font-family:\"DM Mono\",monospace;font-size:1.5rem;"
            f"color:#c8f135;'>{R['initial_cost']:.1f}</div>"
            f"</div>"
            f"<div style='font-size:1.2rem;color:#2e2e3a;'>→</div>"
            f"<div>"
            f"<div style='font-size:0.6rem;color:#454550;letter-spacing:0.16em;"
            f"text-transform:uppercase;'>Optimised</div>"
            f"<div style='font-family:\"DM Mono\",monospace;font-size:1.5rem;"
            f"color:#39e5b6;'>{R['final_cost']:.1f}</div>"
            f"</div>"
            f"<div style='margin-left:auto;text-align:right;'>"
            f"<div style='font-size:0.6rem;color:#454550;letter-spacing:0.16em;"
            f"text-transform:uppercase;'>Distance Saved</div>"
            f"<div style='font-family:\"DM Mono\",monospace;font-size:1.8rem;"
            f"color:#c8f135;font-weight:500;'>{pct:.1f}%</div>"
    ...
    with tabs[2]:
        ...
        if R["route_analytics"]:
            ...
            for k, ra in enumerate(R["route_analytics"]):
                color = VEHICLE_COLORS[k % len(VEHICLE_COLORS)]
                util_w = min(ra["utilisation"], 100)
                with cols[k % min(num_routes, 4)]:
                    st.markdown(
                        f"<div style='background:#06060a;border:1px solid #1a1a28;"
                        f"border-radius:6px;padding:0.9rem;margin-bottom:0.5rem;'>"
                        f"<div style='font-size:0.6rem;font-weight:600;color:{color};"
                        f"letter-spacing:0.16em;text-transform:uppercase;"
                        f"margin-bottom:0.6rem;'>Vehicle {k+1}</div>"
                        f"<div style='display:grid;gap:0.3rem;'>"
                        f"<div style='display:flex;justify-content:space-between;'>"
                        f"<span style='font-size:0.7rem;color:#2e2e3a;'>Cost</span>"
                        f"<span style='font-family:\"DM Mono\",monospace;font-size:0.7rem;"
                        f"color:{color};'>{ra['cost']:.1f}</span></div>"
                        f"<div style='display:flex;justify-content:space-between;'>"
                        f"<span style='font-size:0.7rem;color:#2e2e3a;'>Load</span>"
                        f"<span style='font-family:\"DM Mono\",monospace;font-size:0.7rem;"
                        f"color:{color};'>{ra['demand']:.0f}/{capacity:.0f}</span></div>"
                        f"<div style='display:flex;justify-content:space-between;'>"
                        f"<span style='font-size:0.7rem;color:#2e2e3a;'>Travel</span>"
                        f"<span style='font-family:\"DM Mono\",monospace;font-size:0.7rem;"
                        f"color:{color};'>{ra['travel_time']:.1f}</span></div>"
                        f"<div style='display:flex;justify-content:space-between;'>"
                        f"<span style='font-size:0.7rem;color:#2e2e3a;'>Service</span>"
                        f"<span style='font-family:\"DM Mono\",monospace;font-size:0.7rem;"
                        f"color:{color};'>{ra['service_time']:.1f}</span></div>"
                        f"<div style='display:flex;justify-content:space-between;'>"
                        f"<span style='font-size:0.7rem;color:#2e2e3a;'>Util</span>"
                        f"<span style='font-family:\"DM Mono\",monospace;font-size:0.7rem;"
                        f"color:{color};'>{ra['utilisation']:.0f}%</span></div>"
                        f"</div>"
                        f"<div style='margin-top:0.6rem;background:#0e0e16;"
                        f"border-radius:2px;height:3px;'>"
                        f"<div style='background:{color};width:{util_w:.1f}%;"
            ...
            if pri and enable_priority:
                ...
                st.markdown(
                    ...
                    "Priority Distribution in Routes</div>",
                    unsafe_allow_html=True,
                )
                pri_rows = []
                for k, ra in enumerate(R["route_analytics"]):
                    ...
                    for nd in customers:
                        p = pri[nd - 1]
                        level_label = {1: "Low", 2: "Medium", 3: "High"}[p]
                        pri_rows.append({"Vehicle": k+1, "Node": nd, "Priority": p, "Level": level_label})
                if pri_rows:
                    ...
                    for ki, r in enumerate(entry["routes_after"]):
                        if len(r) > 2:
                            ...
                            (mc1 if ki % 2 == 0 else mc2).markdown(
                                ...
                                f"font-size:0.72rem;color:#e8e6e0;'>{path}</div>"
                                f"<div style='font-size:0.62rem;color:#2e2e3a;"
                                f"margin-top:0.2rem;'>load {d_r:.0f} · cost {c_r:.1f}</div>"
                                f"</div>",
                                unsafe_allow_html=True,
                            )
    ...
    with tabs[4]:
        ...
        labels = ["Depot" if i == 0 else f"N{i}" for i in range(n)]
