...
num_vehicles = col1.number_input("🚚 Vehicles", 1, 20, 3)
vehicle_capacity = col2.number_input("📦 Capacity", 1, 500, 100)
fuel_cost_per_km = col3.number_input("⛽ Fuel Cost/KM", value=50.0)

col4, col5 = st.columns(2)
revenue_per_unit = col4.number_input("💰 Revenue per Unit", value=200.0)
penalty_per_min = col5.number_input("⏱️ Late Delivery Penalty", value=5.0)
    ...
    if st.session_state.routes:
        ...
        for i,(route,_) in enumerate(st.session_state.routes):
            ...
            for node in route:
                ...
                folium.Marker([lat,lon], icon=folium.Icon(color=colors[i%len(colors)])).add_to(m)
